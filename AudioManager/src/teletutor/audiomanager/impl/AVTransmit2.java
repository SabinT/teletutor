...
//							supported[0]);
	...
	SourceDescription srcDesList[] = new SourceDescription[1];
        ...
        // Send only the first stream int the dataOutput
	for (int i = 0; i < pbss.length; i++) {
	    try {
		rtpMgrs[i] = RTPManager.newInstance();	    
                //rtpMgrs[i].getLocalParticipant().setSourceDescription(srcDesList) 
		...
		rtpMgrs[i].initialize(saList, srcDesList, 0.1, 0.5, null);
...
//					     args[i+1], args[i+2], fmt);
        AVTransmit2 at = new AVTransmit2(new MediaLocator("dsound://8000"), "224.144.251.104", "22200", fmt, "Heme");
	// Start the transmission
    ...
    void pause () {
        ...
        try {
            ...
            Logger.getLogger(AVTransmit2.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

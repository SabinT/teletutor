...
public abstract class TeleObject {
    ...
    public final void registerSubChannel(TeleChannel chan) throws Exception {
        if (name == null) {
            throw new Exception("Name of Object has not been set yet");
        }
        if (chan == null) {
            channel = chan;
    ...
    public final void unregisterSubChannel() {
        if (channel != null) {
            channel.unregisterSubChannel(this);
        }
    }
    ...
    protected final void registerFieldChange(String fieldName, Serializable value) {
        // flush automatically if changes have not been flushed for
        if (flushTask == null) {
            flushTask = new FlushTask();
            timer.schedule(flushTask, AUTO_FLUSH_INTERVAL);
        }
     ...
     * Or elsewhere. This method can be overridden to control what happens when
     * an update message is received.
     * @param changes 
     */
    public void update(UpdateInfo changes) {
     ...
     * @param changes 
     */
    protected abstract void objectUpdated(UpdateInfo changes);
...
//            }
    ...
    /**
     * Each TeleObject is provided with a java timer. Can be used to schedule
     * auto-flushing, animations etc.
     */
    protected Timer timer = new Timer();
    private final long AUTO_FLUSH_INTERVAL = 500;
    TimerTask flushTask = null;
